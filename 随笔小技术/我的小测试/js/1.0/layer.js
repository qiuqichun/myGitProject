...
function(a) {
	...
	var k = {
		...
		close: function(a) {
			var b = f("#" + i[0] + a)[0];
			if (b.className.match(new RegExp('(\\s|^)layermshow(\\s|$)'))) {  
				var reg = new RegExp('(\\s|^)layermshow(\\s|$)');
				  
				b.className = b.className.replace(reg, ' '); 
			} 
			setTimeout(function(){
